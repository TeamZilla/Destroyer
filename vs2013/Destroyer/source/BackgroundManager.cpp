...
bool BackgroundManager::Init(Object* bg, Object* fg)
{
	m_playerSpeed = 0;
	m_backSpeed = 0;
	m_frontSpeed = 0;
	...
	m_bgs.push_back(fg->AddChild(new Background(frontTex)).get());
	//Start positions
	m_bgs[0]->transform.SetPosition(-uthEngine.GetWindow().GetSize().x / 2, m_mountainSpawnY);
	m_bgs[1]->transform.SetPosition(m_bgs[0]->transform.GetPosition().x + m_bgs[0]->transform.GetSize().x, m_mountainSpawnY);
	m_bgs[2]->transform.SetPosition(-uthEngine.GetWindow().GetSize().x / 2, m_backSpawnY);
	m_bgs[3]->transform.SetPosition(m_bgs[2]->transform.GetPosition().x + m_bgs[2]->transform.GetSize().x, m_backSpawnY);
	m_bgs[4]->transform.SetPosition(-uthEngine.GetWindow().GetSize().x / 2, m_frontSpawnY);
	m_bgs[5]->transform.SetPosition(m_bgs[4]->transform.GetPosition().x + m_bgs[4]->transform.GetSize().x, m_frontSpawnY);
